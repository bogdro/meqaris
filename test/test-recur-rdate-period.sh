...
# Test for creating a recurring event with an extra RDATE with a period syntax

. test-func.bash

test_log=test-recur-rdate-period.log

uid=`printf test_uid_%06d $1`
resource=room403@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}02T024500"
dtend="$year${month}02T030000"

./create-mail --attendee "$resource:mailto:$resource" \
	--rrule 'FREQ=DAILY;COUNT=5;BYDAY=MO,TU,WE,TH,FR,SA,SU' \
	--rdate "$year${month}12T024500/$year${month}12T031500" \
...
echo $res | grep "$year-$month-02 03:00:00"
echo $res | grep "$year-$month-03 02:45:00"
echo $res | grep "$year-$month-03 03:00:00"
echo $res | grep "$year-$month-04 02:45:00"
echo $res | grep "$year-$month-04 03:00:00"
echo $res | grep "$year-$month-05 02:45:00"
echo $res | grep "$year-$month-05 03:00:00"
echo $res | grep "$year-$month-06 02:45:00"
echo $res | grep "$year-$month-06 03:00:00"
# no more occurrences:
(echo $res | grep "$year-$month-07 02:45:00") && exit 1
(echo $res | grep "$year-$month-07 03:00:00") && exit 2
# the extra RDATE:
echo $res | grep "$year-$month-12 02:45:00"
echo $res | grep "$year-$month-12 03:15:00"
