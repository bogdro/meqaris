...
# Test for creating an event with a resource as optional

. test-func.bash

test_log=test-role-opt.log

uid=test_uid_000007
resource=room403@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}01T141500"
dtend="$year${month}01T143000"

./create-mail --attendee "$resource:mailto:$resource" \
	--dtstart $dtstart --dtend $dtend --from "$from" \
	--organizer "$organizer" --subject "$subject" \
	--to "blah <$resource>" --uid $uid --role 'OPT-PARTICIPANT' \
| \
$meqaris > $test_log
