...
echo $res | grep "$year-$month-03 01:30:00"
...
# no more occurrences:
(echo $res | grep "$year-$month-07 01:15:00") && exit 1
(echo $res | grep "$year-$month-07 01:30:00") && exit 2
# the extra RDATE:
echo $res | grep "$year-$month-12 01:15:00"
echo $res | grep "$year-$month-12 01:30:00"
