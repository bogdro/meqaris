...
# 1 recurrence for one of them

. test-func.bash

test_log=test-recur-multi-cancel-1res-1rec.log

uid=test_uid_000029
resource1=room403@localhost
resource2=room404@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}02T044500"
dtend="$year${month}02T050000"
...
./create-mail --attendee "$resource2:mailto:$resource2" \
	--method CANCEL --recur-id "$year${month}05T044500" \
	--dtstart $dtstart --dtend $dtend --from "$from" \
	--organizer "$organizer" --subject "$subject" \
...
echo $res | grep "$year-$month-02 04:45:00"
echo $res | grep "$year-$month-02 05:00:00"
echo $res | grep "$year-$month-03 04:45:00"
echo $res | grep "$year-$month-03 05:00:00"
echo $res | grep "$year-$month-04 04:45:00"
echo $res | grep "$year-$month-04 05:00:00"
# the 5th is NOT cancelled for resource1:
echo $res | grep "$year-$month-05 04:45:00"
echo $res | grep "$year-$month-05 05:00:00"
echo $res | grep "$year-$month-06 04:45:00"
echo $res | grep "$year-$month-06 05:00:00"
# no more occurrences:
(echo $res | grep "$year-$month-07 04:45:00") && exit 1
