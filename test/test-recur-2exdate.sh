...
# Test for creating a recurring event with 2 EXDATEs

. test-func.bash

test_log=test-recur-2exdate.log

uid=`printf test_uid_%06d $1`
resource=room403@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}02T031500"
dtend="$year${month}02T033000"

./create-mail --attendee "$resource:mailto:$resource" \
	--rrule 'FREQ=DAILY;COUNT=5;BYDAY=MO,TU,WE,TH,FR,SA,SU' \
	--exdate "$year${month}03T031500,$year${month}05T033000" \
...
echo $res | grep "$year-$month-02 03:15:00"
echo $res | grep "$year-$month-02 03:30:00"
# the 3rd is excluded:
echo $res | grep "$year-$month-04 03:15:00"
echo $res | grep "$year-$month-04 03:30:00"
# the 5th is excluded:
echo $res | grep "$year-$month-06 03:15:00"
echo $res | grep "$year-$month-06 03:30:00"
# no more occurrences:
(echo $res | grep "$year-$month-07 03:15:00") && exit 1
(echo $res | grep "$year-$month-07 03:30:00") && exit 2
