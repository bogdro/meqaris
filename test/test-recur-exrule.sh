...
# Test for creating a recurring event with an EXRULE

. test-func.bash

test_log=test-recur-exrule.log

uid=`printf test_uid_%06d $1`
resource=room403@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}02T014500"
dtend="$year${month}02T020000"
...
echo $res | grep "$year-$month-05 02:00:00"
echo $res | grep "$year-$month-06 01:45:00"
echo $res | grep "$year-$month-06 02:00:00"
# no more occurrences:
(echo $res | grep "$year-$month-07 01:45:00") && exit 1
(echo $res | grep "$year-$month-07 02:00:00") && exit 2
