...
# Test for creating an event with a DURATION

. test-func.bash

test_log=test-duration.log

uid=test_uid_000003
resource=room403@localhost
subject="Event $uid $RANDOM"
dtstart="$year${month}01T130000"

./create-mail --attendee "$resource:mailto:$resource" \
	--dtstart $dtstart --duration PT1H --from "$from" \
	--organizer "$organizer" --subject "$subject" \
	--to "test3 <$resource>" --uid $uid \
| \
$meqaris > $test_log
