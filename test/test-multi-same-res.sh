...
# Test for creating an event for the same resource multiple times

. test-func.bash

test_log=test-multi-same-res.log
...
./create-mail --attendee "$resource:mailto:$resource","$resource:mailto:$resource2" \
	--dtstart $dtstart --dtend $dtend --from "$from" \
	--organizer "$organizer" --subject "$subject" \
	--to "room403 <$resource>","room403 <$resource>" --uid $uid \
| \
$meqaris > $test_log
